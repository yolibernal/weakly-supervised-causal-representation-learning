...
def create_model(cfg):
    ...
    elif cfg.model.type == "sequence_intervention_noise_vae":
        intervention_encoder = create_intervention_encoder(cfg)
        model = MonotonousEffectSequenceILCM(
            scm,
            encoder=encoder,
            decoder=decoder,
            intervention_encoder=intervention_encoder,
...
def create_scm(cfg):
    ...
    noise_centric = cfg.model.type in {
        ...
        "sequence_intervention_noise_vae",
    }
    ...
    for epoch in epoch_generator:
        ...
        for x1, x2, z1, z2, intervention_labels, true_interventions in train_loader:
            ...
            loss, metrics = criteria(
                ...
                intervened_sequence_regularization_amount=intervened_sequence_regularization_amount,
                unintervened_sequence_regularization_amount=unintervened_sequence_regularization_amount,
                **model_outputs,
            )
...
def load_dataset(cfg, tag, include_noise_encodings=True, normalize=True, sequence=True):
    ...
    x1, x2, z1, z2, intervention_label, intervention_mask, e1, e2 = data

    device = x1.device

    # TODO: fix normalization for sequence data
    if normalize:
    ...
    if len(x2.shape) == 2:
        # If not sequence, turn into sequence of length 1
        x2 = x2.unsqueeze(1)
        z2 = z2.unsqueeze(1)
        e2 = e2.unsqueeze(1)

    if x2.shape[1] > 1 and cfg.data.sequence_length == 1:
        logger.info("Using only last sequence element because sequence length is set to 1")
        x2 = x2[:, -1].unsqueeze(1)
        z2 = z2[:, -1].unsqueeze(1)
        e2 = e2[:, -1].unsqueeze(1)
    ...
    if include_noise_encodings:
        dataset = TensorDataset(x1, x2, z1, z2, intervention_label, intervention_mask, e1, e2)
    else:
        dataset = TensorDataset(x1, x2, z1, z2, intervention_label, intervention_mask)
    return dataset
...
def plot_loop(cfg, model, loader, metrics, device, step):
    ...
    with torch.no_grad():
        ...
        # plot_intervention_posterior_scatter_plot?
        # plot_noise_encoding_distributions?
...
def eval_enco_graph(cfg, model, partition="train"):
    ...
    with torch.no_grad():
        for x0, x1_sequence, *_ in dataloader:
            x0, x1_sequence = x0.to(device), x1_sequence.to(device)
            _, _, _, _, e0, e1_sequence, _, _, intervention = model.encode_decode_pair(
                x0.to(device), x1_sequence.to(device)
            )
            e1 = e1_sequence[:, -1]
...
def fix_topological_order(cfg, model, partition="val", dataloader=None):
    ...
    # This is only defined for noise-centric models (ILCMs)
    assert cfg.model.type in ["intervention_noise_vae", "sequence_intervention_noise_vae"]
